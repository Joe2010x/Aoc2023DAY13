...
public class Map
{
    ...
    public Map(List<string> lines)
    {
        ...
        var colArr = Enumerable
            .Range(0, Col)
            .Select(col => Enumerable.Range(0, Row).Select(row => rowArr[row][col]).ToArray())
            .ToList();
        RowArr = rowArr;
        ColArr = colArr;
    ...
    {
        Value =
            RowRelection.Aggregate(0, (a, b) => a + b) * 100
            + 1 * ColRelection.Aggregate(0, (a, b) => a + b);
        ChangedValue =
            NewRowRelection.Aggregate(0, (a, b) => a + b) * 100
            + 1 * NewColRelection.Aggregate(0, (a, b) => a + b);
    }
